...
export default function SplitsPage({ userEmail, authToken, transactions }: SplitsPageProps) {
  ...
  const fetchGroups = useCallback(async () => {
    ...
    try {
      ...
      setGroups((result.groups as SplitGroup[]) ?? []);
    } catch (err) {
  ...
  const fetchExpenses = useCallback(async (groupId: string) => {
    ...
    try {
      ...
      setExpenses((result.expenses as SplitExpense[]) ?? []);
    } catch (err) {
